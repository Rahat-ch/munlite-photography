  ...
  render() {
    ...
    return (
      <Fragment>
        <Gallery>
          {images.map((img, i) => (
            <GalleryItem key={img.node.title}>
              <a href={img.node.image.asset.fluid.src} alt={img.node.title} onClick={e => this.handleClick(e, i)}>
                <StyledImg sizes={img.node.image.asset.fluid} />
              </a>
            </GalleryItem>
          ))}
        </Gallery>
        ...
        <LightboxModal visible={showLightbox} onKeyUp={e => this.handleKeyDown(e)}>
          <LightboxContent>
            <Img sizes={images[selectedImage].node.image.asset.fluid} />
            <Controls>
